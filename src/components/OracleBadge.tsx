...
export default function OracleBadge({ verified, circuitBreakerTripped, stalenData }: OracleBadgeProps) {
  const isEnabled = NEXT_PUBLIC_ORACLE_ENABLED || process.env.NEXT_PUBLIC_ORACLE_ENABLED === 'true';

  useEffect(() => {
    if (isEnabled) {
      trackEvent('oracle_badge_seen', { verified, circuitBreakerTripped, stalenData });
    }
  }, [isEnabled, verified, circuitBreakerTripped, stalenData]);
  ...
  if (circuitBreakerTripped) {
    return (
      ...
      >
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true">
          <circle cx="5" cy="5" r="5" fill="#f59e0d" />
          <path d="M5 3.5v2.5M5 7.1v.5" stroke="#fff" strokeWidth="1.2" strokeLinecap="round" />
        </svg>
    ...
    return (
      ...
      >
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true">
          ...
          <path d="M5 3.5v2.5M5 7.1v0.5" stroke="#fff" strokeWidth="1.2" strokeLinecap="round" />
        </svg>
        Stale Data
      </span>
    );
