 ...
 * (ORACLE, NFT, INSURANCE_POOL) against the invoice detail page and
 * individual gated components. Ensures no combination causes rendering
 * errors, missing elements, or cross-flag interference.
 *
...
describe('Multi-flag rendering isolation', () => {
  ...
  it('oracle + nft: both render independently without overlap', () => {
    ...
    expect(screen.getByText('Oracle Verified')).toBeInTheDocument();
    expect(screen.getByText(/Invoice NFT/)).toBeInTheDocument();
    // No error thrown, both components render in the same tree.
