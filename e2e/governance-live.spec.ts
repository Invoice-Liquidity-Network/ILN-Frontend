...
// Helper for network resilience with retries
...
test.describe('Live testnet governance vote journey', () => {
  ...
  test('can navigate to governance proposals page and see active proposals', async ({ page }) => {
    await loadPageWithRetry(page, '/governance');
    ...
    // Check for governance/proposal headings with increased timeout
    const heading = page
      .locator('h1, h2')
    ...
    await expect(proposalElements).toBeVisible({ timeout: 15000 });
  });
  ...
  test('displays voting options on an active proposal', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      await expect(voteButtons.first()).toBeVisible({ timeout: 15000 });
    }
  });

  test('proposal detail page loads with expected elements', async ({ page }) => {
    // Navigate directly to proposal 1 as a baseline
    const response = await loadPageWithRetry(page, '/governance/1');
  ...
  test('vote submission requires wallet connection', async ({ page }) => {
    ...
    const voteButton = page
      ...
      .first();
    const isVoteButtonVisible = await voteButton.isVisible({ timeout: 10000 }).catch(() => false);
  ...
  test('displays vote counts and proposal status', async ({ page }) => {
    await loadPageWithRetry(page, '/governance/1');

    const main = page.locator('main').first();
    const isMainVisible = await main.isVisible({ timeout: 30000 }).catch(() => false);

    if (isMainVisible) {
      // Check for vote count indicators
      const voteCounter = page.locator('text=/votes?|for|against|abstain|\d+%/i').first();
      const isCounterVisible = await voteCounter.isVisible({ timeout: 15000 }).catch(() => false);
      ...
      const isStatusVisible = await statusElement.isVisible({ timeout: 15000 }).catch(() => false);
  ...
  test('proposal page is resilient to missing data', async ({ page }) => {
    ...
    await loadPageWithRetry(page, '/governance/99999');
