...
A `git filter-repo` history rewrite was performed to purge `storybook-static/`, `test-results/`, `build-storybook.log`, and `debug-storybook.log` from all commits. This reduced the clone size from ~46 MiB to ~12 MiB — a **74% reduction**.
...
3. **CI check** (`.github/workflows/ci.yml`) now fails if any build-artifact-shaped file is staged for commit. The check scans staged files against patterns: `storybook-static/`, `test-results/`, `playwright-report/`, `build-storybook.log`, `debug-storybook.log`.
